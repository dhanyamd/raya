...
const SaveRequestToCollectionModal = ({
  ...
  const requestColorMap: Record<REST_METHOD, string> = {
    ...
    [REST_METHOD.POST]: "text-brown-500",
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            <input
              className="w-full p-3 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-brown-500 focus:border-transparent pr-20"
        ...
        <div>
          ...
          <div className="relative mb-3">
            ...
            <input
              type="text"
              placeholder="Search"
              className="w-full bg-zinc-800 border border-zinc-700 rounded-lg pl-10 pr-4 py-2 text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-brown-500 focus:border-transparent"
              value={searchTerm}
          ...
          <div className="space-y-1 max-h-48 overflow-y-auto">
            {isLoading ? (
              <div className="flex items-center justify-center py-8">
                <div className="w-5 h-5 border-2 border-zinc-600 border-t-brown-500 rounded-full animate-spin"></div>
                ...
                <div
                  ...
                  className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-all duration-200 ${
                    selectedCollectionId === collection.id
                      ? "bg-brown-600/20 border border-brown-500/50 shadow-lg shadow-brown-500/10"
                      : "hover:bg-zinc-800 border border-transparent"
                  }`}
                >
                  <div className="flex items-center space-x-3">
                    {selectedCollectionId === collection.id ? (
                      <div className="w-4 h-4 rounded-full bg-brown-500 flex items-center justify-center">
                    ...
                    <span className={`text-sm font-medium ${
                      selectedCollectionId === collection.id ? "text-brown-200" : "text-zinc-200"
                    }`}>
                      {collection.name}
                    </span>
                  </div>
        ...
        {selectedCollection && (
          <div className="p-3 bg-zinc-800/50 rounded-lg border border-zinc-700">
            <div className="flex items-center space-x-2 text-sm">
              ...
              <Folder className="w-4 h-4 text-brown-400" />
              <span className="text-brown-400 font-medium">{selectedCollection.name}</span>
            </div>
          </div>
        )}
